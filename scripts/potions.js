var potions = {
    ...
    buyPotions : function(potion, price, quantity) {
        this.getPotions(potion, quantity);
        candies.setNbrOwned(candies.nbrOwned - price);
        shop.setMerchantSpeech("So many candiiiiiiiiiiiiies!!!");
    },

    buyScrollSet : function(price, quantity) {
        var list = htmlInteraction.getElement("scroll_set_type");
        for (var potion in potions.list) {
            if (potions.list[potion].buttonText === (list.value + ' scroll')) {
                this.buyPotions(potions.list[potion], price, quantity);
            }
        }
    },
